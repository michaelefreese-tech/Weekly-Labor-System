...
function ApplySheetProtections() {
  ...
  Info._META.REQUIRED_SHEETS.forEach(sheetName => {
...
function ViewChangeLog() {
  ...
  try {
    const rawLog = props.getProperty("SCRIPT_CHANGELOG_DICT") || "[]";
    logList = JSON.parse(rawLog);
  } catch (err) {
    ui.alert("Error reading changelog data.");
    return;
  }
  ...
  const trimmed = logList.slice().reverse().slice(0, MAX_ENTRIES);
  ...
  if (logList.length > MAX_ENTRIES) {
    message += `...and ${logList.length - MAX_ENTRIES} more entries`;
  }
...
function ClearVersionHistory() {
  ...
  const confirmation = ui.alert(
    "⚠️ WARNING",
    "This will permanently delete version history. Proceed?",
    ui.ButtonSet.YES_NO
  );
