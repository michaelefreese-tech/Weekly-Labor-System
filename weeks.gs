...
function GetWeekInfo(baseDate = null) {
  ...
  const rawDate = infoSheet.getRange("E7").getValue();
  ...
  const periodNumber = infoSheet.getRange("G7").getValue();
...
function CheckDate(targetIndex) {
  ...
  const currentPeriod = parseInt(infoSheet.getRange("G7").getValue(), 10) || 1;
  let direction;
  let periodStart;
  let period;
  ...
  infoSheet.getRange("E7").setValue(periodStart);
  infoSheet.getRange("G7").setValue(period);
...
function UpdateWeekView(index) {
  ...
  info.getRange("G15").setValue(week[0]);
  info.getRange("H15").setValue(":");
  info.getRange("I15").setValue(week[6]);
...
function UpdateInfoSheet() {
  ...
  weekKeys.forEach((label, i) => {
    ...
    info.getRange("E" + (10 + i)).setValue(label);
    info.getRange("G" + (10 + i)).setValue(weekRange[0]);
    info.getRange("H" + (10 + i)).setValue(":");
    info.getRange("I" + (10 + i)).setValue(weekRange[6]);
  });
  ...
  if (periodStart) {
    info.getRange("E7").setValue(new Date(periodStart));
  }

  if (periodNumber) {
    info.getRange("G7").setValue(parseInt(periodNumber, 10));
  }
}
